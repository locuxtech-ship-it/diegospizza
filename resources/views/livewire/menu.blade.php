<div class="min-h-screen bg-white text-gray-900 font-sans" style="font-family: 'Poppins', sans-serif;">
    ...
    <div class="relative text-white overflow-hidden bg-gray-900" style="background-image: linear-gradient(to bottom, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.6) 50%, rgba(0,0,0,0.5) 100%), url('/storage/images/llanera.jpeg'); background-size: cover; background-position: center;">
        <div class="max-w-4xl mx-auto px-4 py-5 sm:py-10">
            <div class="flex items-center justify-between gap-3">
                <div class="flex items-center gap-3 min-w-0">
                    @if($negocio->logo)
                        <img src="{{ asset('storage/' . $negocio->logo) }}" alt="{{ $negocio->nombre_negocio }}" class="h-11 sm:h-14 rounded-xl shadow-lg border-2 border-white/20 flex-shrink-0">
                    @else
                        <div class="h-11 sm:h-14 w-11 sm:w-14 bg-orange-500/20 rounded-full flex items-center justify-center border-2 border-white/20 flex-shrink-0">
                            <span class="text-xl sm:text-3xl">🍕</span>
                        </div>
                    @endif
                    <div class="min-w-0">
                        <h1 class="text-base sm:text-2xl font-bold tracking-tight truncate drop-shadow-lg">
                            {{ $negocio->nombre_negocio ?? "Diego's Pizza" }}
                        </h1>
                        <p class="text-gray-300 text-xs truncate drop-shadow">Bienvenido a Diego's Pizza Alameda</p>
                    </div>
                </div>

                <div class="flex items-center gap-1.5 bg-black/50 backdrop-blur-sm px-2.5 py-1.5 rounded-full border border-white/20 flex-shrink-0">
            ...
            <div class="flex flex-wrap items-center gap-x-2.5 gap-y-1 mt-3 sm:mt-4 text-xs text-gray-300">
