<div>
    ...
    <div class="bg-gradient-to-br from-red-700 via-red-600 to-orange-500 text-white">
        <div class="max-w-4xl mx-auto px-4 py-6">
            <a href="{{ route('menu') }}" wire:navigate class="inline-flex items-center text-red-100 hover:text-white mb-4 transition">
    ...
    <div class="max-w-4xl mx-auto px-4 py-6 sm:py-8">
        @if($pedidoCreado)
            <div class="text-center py-12 max-w-md mx-auto">
                ...
                <p class="text-gray-500 mb-2">Tu pedido <strong class="text-red-600">#{{ $pedidoId }}</strong> ha sido recibido</p>
                <div class="bg-gray-50 rounded-xl p-4 mb-4 text-sm space-y-1">
                    <p class="text-gray-600">Total: <strong class="text-gray-800">${{ number_format($total, 0, ',', '.') }}</strong></p>
                    <p class="text-gray-600">Pago: <strong class="text-gray-800">{{ ucfirst($metodoPago) }}</strong></p>
                </div>

                @if($metodoPago === 'transferencia')
                    @php $s = \App\Models\NegocioSetting::getSettings(); @endphp
                ...
                <a href="{{ route('menu') }}" wire:navigate class="inline-block bg-gradient-to-r from-red-600 to-red-500 text-white px-8 py-3 rounded-xl font-bold hover:from-red-700 hover:to-red-600 transition-all active:scale-95 shadow-sm w-full">
                    Seguir ordenando
                </a>
            </div>
        @else
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6 lg:gap-8">
                ...
                <div class="lg:col-span-3 space-y-4 sm:space-y-6">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="tel" wire:model.live="telefono" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 @error('telefono') border-red-300 bg-red-50 @enderror" placeholder="555-1234-5678" autofocus>
                                @error('telefono') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nombre *</label>
                                <input type="text" wire:model.live="nombre" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 @error('nombre') border-red-300 bg-red-50 @enderror" placeholder="Tu nombre">
                            ...
                            <div class="bg-orange-50 border border-orange-100 rounded-xl p-4 text-sm space-y-3">
                                <div class="flex gap-4 items-center flex-wrap">
                                    <span class="px-2 py-0.5 rounded-lg text-xs font-bold"
                                        style="background: {{ $clienteInfo['clasificacion'] === 'elite' ? '#fef3c7' : ($clienteInfo['clasificacion'] === 'frecuente' ? '#ffedd5' : '#f3f4f6') }}; color: {{ $clienteInfo['clasificacion'] === 'elite' ? '#92400e' : ($clienteInfo['clasificacion'] === 'frecuente' ? '#9a3412' : '#6b7280') }};">
                                ...
                                @if(!empty($clienteInfo['recompensas']))
                                    <div style="border-top: 1px dashed #fed7aa; padding-top: 8px;">
                                        ...
                                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 6px 10px; border-radius: 8px; font-size: 13px; {{ $recompensaSeleccionadaIndex === null ? 'background: #fef3c7; font-weight: 700;' : '' }}">
                                            <input type="radio" name="recompensa" wire:click="seleccionarRecompensa(null)" {{ $recompensaSeleccionadaIndex === null ? 'checked' : '' }} style="accent-color: #ea580c;">
                                            <span>❌ No usar puntos</span>
                                        </label>
                                        @foreach($clienteInfo['recompensas'] as $i => $r)
                                            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 6px 10px; border-radius: 8px; font-size: 13px; {{ $recompensaSeleccionadaIndex === $i ? 'background: #fef3c7; font-weight: 700;' : '' }}">
                                                <input type="radio" name="recompensa" wire:click="seleccionarRecompensa({{ $i }})" {{ $recompensaSeleccionadaIndex === $i ? 'checked' : '' }} style="accent-color: #ea580c;">
                                                <span>{{ number_format($r['puntos']) }} pts →</span>
                                                @if($r['tipo'] === 'porcentaje')
                                                    <span style="background: #fef3c7; padding: 1px 8px; border-radius: 9999px; font-weight: 700;">{{ $r['valor'] }}% OFF</span>
                                                @else
                                                    <span style="background: #fef3c7; padding: 1px 8px; border-radius: 9999px; font-weight: 700;">${{ number_format($r['valor'], 0, ',', '.') }} OFF</span>
                                                @endif
                                            </label>
                                        @endforeach
                                    </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <input wire:model.live="conjunto" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500 @error('conjunto') border-red-300 bg-red-50 @enderror" placeholder="Nombre del conjunto">
                                    @error('conjunto') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                </div>
                                <div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            @php $metodosPago = \App\Models\NegocioSetting::getActivePaymentMethods(); @endphp
                            @foreach($metodosPago as $valor => $info)
                                <label class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 {{ $metodoPago === $valor ? 'border-red-500 bg-red-50' : 'border-gray-100 hover:border-gray-200 bg-gray-50' }}">
                                    <input type="radio" wire:model.live="metodoPago" value="{{ $valor }}" class="text-red-600 focus:ring-red-500 sr-only">
                                    <span class="text-xl">{{ $info['icon'] }}</span>
                                    <span class="font-medium text-sm text-gray-800">{{ $info['label'] }}</span>
                                </label>
                            @endforeach
                        </div>
                        @error('metodoPago') <span class="text-red-500 text-xs mt-2">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 sticky top-4">
                        ...
                        <a href="{{ route('menu') }}" wire:navigate class="w-full mt-3 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all duration-200 active:scale-[0.98] border" style="border-color: #FF8D08; color: #FF8D08;">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                            Agregar más productos
                        </a>
                        <button wire:click="procesarPedido" wire:loading.attr="disabled"
                            class="w-full mt-3 bg-gradient-to-r from-red-600 to-red-500 text-white py-3 rounded-xl font-bold hover:from-red-700 hover:to-red-600 transition-all duration-200 disabled:opacity-50 active:scale-[0.98] shadow-sm">
