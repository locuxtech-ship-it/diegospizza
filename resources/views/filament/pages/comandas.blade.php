        ...
        <div style="background: white; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
            ...
            <div wire:click="editarPedido({{ $pedido['id'] }})" style="display: grid; grid-template-columns: 1.3fr 0.8fr 0.7fr 1.4fr auto; gap: 0; border-bottom: 1px solid #f3f4f6; font-size: 13px; cursor: pointer; transition: background 0.15s;"
                ...
                <div style="padding: 6px 8px; text-align: center; display: flex; gap: 3px; align-items: center; justify-content: center; flex-wrap: wrap;">
                    ...
                    <a href="/admin/pedidos/{{ $pedido['id'] }}/edit" style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 5px; font-size: 12px; text-decoration: none; background: #f3f4f6; color: #374151; border: 1px solid #e5e7eb;" title="Editar pedido">✏️</a>
                    @endif
                    <a href="#" onclick="event.stopPropagation(); printPedido({{ $pedido['id'] }}); return false;" style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 5px; font-size: 12px; font-weight: 500; text-decoration: none; background: #f3f4f6; color: #374151; border: 1px solid #e5e7eb;" title="Reimprimir ticket">🖨️</a>
                    @if(!empty($pedido['cliente']['telefono']))
                        @php $waTel = preg_replace('/[^0-9]/', '', $pedido['cliente']['telefono']); @endphp
                        <a href="https://wa.me/{{ $waTel }}?text={{ urlencode('Hola! Te escribimos de Diego\'s Pizza por tu pedido #' . $pedido['numero_pedido']) }}" target="_blank" style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 5px; font-size: 12px; font-weight: 500; text-decoration: none; background: #dcfce7; color: #25D366; border: 1px solid #bbf7d0;" title="WhatsApp cliente">
                    ...
                    @can('applyDiscount', auth()->user())
                        <button wire:click.stop="eliminarPedido({{ $pedido['id'] }})" onclick="return confirm('¿Eliminar pedido #{{ $pedido['numero_pedido'] }}? Esta acción no se puede deshacer.')" style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 5px; font-size: 12px; background: #fef2f2; color: #dc2626; border: 1px solid #fecaca; cursor: pointer;" title="Eliminar pedido">🗑️</button>
                    @endcan
