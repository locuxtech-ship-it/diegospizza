<x-filament-panels::page>
    <div style="display: flex; flex-direction: column; gap: 24px;" wire:poll.keep-alive.5s="cargarPedidos">
        ...
        <div style="background: white; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden;">
            <div style="display: grid; grid-template-columns: 40px 1fr 0.8fr 1fr 50px 65px 55px 50px 95px auto; gap: 0; background: #f9fafb; border-bottom: 1px solid #e5e7eb; font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em;">
            ...
            @endphp
            <div wire:click="editarPedido({{ $pedido['id'] }})" style="display: grid; grid-template-columns: 40px 1fr 0.8fr 1fr 50px 65px 55px 50px 95px auto; gap: 0; border-bottom: 1px solid #f3f4f6; font-size: 13px; cursor: pointer; transition: background 0.15s;"
                ...
                <div style="padding: 6px 8px; text-align: center; display: flex; gap: 3px; align-items: center; justify-content: center; flex-wrap: wrap;">
                    ...
                    <a href="/admin/pedidos/{{ $pedido['id'] }}/edit" style="display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 4px; font-size: 10px; text-decoration: none; background: #f3f4f6; color: #374151; border: 1px solid #e5e7eb;" title="Editar pedido">✏️</a>
                    @endif
                    <a href="#" onclick="event.stopPropagation(); printPedido({{ $pedido['id'] }}); return false;" style="display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 4px; font-size: 10px; font-weight: 500; text-decoration: none; background: #f3f4f6; color: #374151; border: 1px solid #e5e7eb;" title="Reimprimir ticket">🖨️</a>
                    @if(!empty($pedido['cliente']['telefono']))
                        @php $waTel = preg_replace('/[^0-9]/', '', $pedido['cliente']['telefono']); @endphp
                        <a href="https://wa.me/{{ $waTel }}?text={{ urlencode('Hola! Te escribimos de Diego\'s Pizza por tu pedido #' . $pedido['numero_pedido']) }}" target="_blank" style="display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 4px; font-size: 10px; font-weight: 500; text-decoration: none; background: #dcfce7; color: #25D366; border: 1px solid #bbf7d0;" title="WhatsApp cliente">
                    ...
                    @can('applyDiscount', auth()->user())
                        <button wire:click.stop="eliminarPedido({{ $pedido['id'] }})" onclick="return confirm('¿Eliminar pedido #{{ $pedido['numero_pedido'] }}? Esta acción no se puede deshacer.')" style="display: inline-flex; align-items: center; justify-content: center; width: 22px; height: 22px; border-radius: 4px; font-size: 10px; background: #fef2f2; color: #dc2626; border: 1px solid #fecaca; cursor: pointer;" title="Eliminar pedido">🗑️</button>
                    @endcan
                    @if($siguiente === 'en_proceso')
                        <x-filament::button wire:click.stop="cambiarEstado({{ $pedido['id'] }}, 'en_proceso')" size="xs" color="success">
                            ✅ Acep.
                        </x-filament::button>
                        <x-filament::button wire:click.stop="rechazarPedido({{ $pedido['id'] }})" size="xs" color="danger">
                            ❌ Rech.
                        </x-filament::button>
                    @elseif($siguiente === 'finalizado')
                        @php $pagoOk = $this->pagoCompleto($pedido['id']); @endphp
                        @if($pagoOk)
                            <x-filament::button wire:click.stop="finalizarPedido({{ $pedido['id'] }})" size="xs" color="success">
                                ✅ Final.
                            </x-filament::button>
                        @else
                            <x-filament::button wire:click.stop="finalizarPedido({{ $pedido['id'] }})" size="xs" color="warning">
                                💳 Pagar
                            </x-filament::button>
                        @endif
                    @else
                        <x-filament::button wire:click.stop="cambiarEstado({{ $pedido['id'] }}, '{{ $siguiente }}')" size="xs" color="gray">
                            {{ $siguiente == 'en_camino' ? '🚗' : '📍' }}
                        </x-filament::button>
                    @endif
                    @can('applyDiscount', auth()->user())
                        @if(!in_array($pedido['estado'], ['pendiente_pago', 'finalizado', 'cancelado']))
                        <x-filament::button wire:click.stop="cancelarPedido({{ $pedido['id'] }})" wire:confirm="¿Cancelar pedido #{{ $pedido['numero_pedido'] }}?" size="xs" color="danger">
                            🛑
                        </x-filament::button>
                        @endif
                    @endcan
                            ...
                            <div wire:click="editarPedido({{ $pedido['id'] }})" style="background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); cursor: pointer;">
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center; gap: 6px; margin-top: 12px; padding-top: 12px; border-top: 1px solid #e5e7eb;">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 4px;">
                                        ...
                                        @endcan
                                        <x-filament::button wire:click.stop="abrirModalPago({{ $pedido['id'] }})" size="xs" color="gray">
                                            💳
                                        </x-filament::button>
                                        ...
                                        @elseif($col['siguiente'] !== 'finalizado')
                                            <x-filament::button wire:click.stop="cambiarEstado({{ $pedido['id'] }}, '{{ $col['siguiente'] }}')" size="xs" color="gray">
                                                {{ $col['accion'] }}
                                            </x-filament::button>
